...
const QUERY_LOJA = (dataInicio, dataFim) => `
WITH vendas_semana AS (
  ...
  WHERE
    ($1::date IS NULL OR v.lastchangedate::date >= $1::date)
    AND ($2::date IS NULL OR v.lastchangedate::date <= $2::date)
  GROUP BY
    v.loja,
    c.semana,
...
const QUERY_LOJA_VENDEDOR = (dataInicio, dataFim) => `
WITH vendas_semana AS (
  SELECT
    v.loja,
    v.seller_name,
...
export default async function handler(req, res) {
  try {
    ...
    const { dataInicio = null, dataFim = null } = req.query;

    const resultLoja = await pool.query(QUERY_LOJA(dataInicio, dataFim), [dataInicio, dataFim]);
    const resultLojaVendedor = await pool.query(QUERY_LOJA_VENDEDOR(dataInicio, dataFim), [dataInicio, dataFim]);
