...
function extractDanfeDataFromXml(xmlObj) { 
  ...
  const nfeProc = xmlObj?.nfeProc || xmlObj?.nfeProcNFe || xmlObj || {}; 
  ...
  try { 
    ...
    if (xmlObj) { 
      ...
      try { 
        ...
        console.error("Falha ao extrair dados do xml_raw para DANFE:", err); 

      } 

    } else { 

      console.log("xml_raw não pôde ser interpretado; usando dados do banco."); 
