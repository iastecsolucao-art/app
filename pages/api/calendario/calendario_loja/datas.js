...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (method === "PUT") {
      ...
      // opcional: garantir que data pertence ao ano
      if (parseInt(data.slice(0, 4), 10) !== anoInt) {
        return res.status(400).json({ error: "A 'data' não pertence ao 'ano' informado" });
      }

      // meta é NUMERIC(10,2) no banco: aceitar número ou null
      let metaNum = null;
      if (meta !== undefined && meta !== null && meta !== "") {
        const parsed = Number(meta);
        if (Number.isNaN(parsed)) {
          return res.status(400).json({ error: "Campo 'meta' inválido (deve ser número)" });
        }
        metaNum = parsed;
      }

      // sua tabela tem UNIQUE (ano, semana, data) => melhor usar ON CONFLICT
      await pool.query(
        `
          INSERT INTO calendario (ano, semana, data, meta)
          VALUES (
            $1::int,
            EXTRACT(ISOWEEK FROM $2::date)::int,
            $2::date,
            $3::numeric
          )
      ...
      return res.status(200).json({ message: `Data ${data} cadastrada/atualizada.` });
    }
    ...
    if (method === "DELETE") {
      ...
      return res.status(200).json({
        message:
          result.rowCount > 0 ? `Data ${data} removida.` : `Data ${data} não existia.`,
      });
    }
