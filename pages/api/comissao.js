...
// VIEW usa: sale_date (não existe invoicedate/issuedate nela)
// Mantém compatibilidade: se alguém enviar invoicedate/issuedate, mapeia pra sale_date.
const DATECOL_MAP = Object.freeze({
  sale_date: "sale_date",
  invoicedate: "sale_date",
  issuedate: "sale_date",
});

// Failsafe definitivo: sempre retorna sale_date se vier qualquer coisa diferente
const sanitizeDateCol = (v) => {
  const key = String(v || "").toLowerCase().trim();
  return DATECOL_MAP[key] || "sale_date";
};

export default async function handler(req, res) {
  try {
    // ===== parâmetros =====
    const anos = parseCsv(req.query.ano).map((x) => parseInt(x, 10)).filter(Number.isFinite);
    const semanasISO = parseCsv(req.query.semana).map((x) => parseInt(x, 10)).filter(Number.isFinite);
    const lojas = parseCsv(req.query.loja);
    const vendedores = parseCsv(req.query.vendedor);

    // IMPORTANTE: default é sale_date
    const VV_DATE_COL = sanitizeDateCol(req.query.datecol || "sale_date");

    // logs (remova depois)
    console.log("[api/comissao] url:", req.url);
    console.log("[api/comissao] VV_DATE_COL:", VV_DATE_COL);

    if (!semanasISO.length) {
      return res.status(200).json({
        data: [],
        resumo_semanal: [],
        subtotais_loja: [],
        total_geral: { meta_total: 0, real_total: 0, comissao_total: 0, pct_meta: 0 },
        datecol: VV_DATE_COL,
      });
    }
    ...
    // ================= MAIN =================
    // Failsafe extra: mesmo que VV_DATE_COL viesse errado, ainda forçamos para sale_date
    // porque só existe essa coluna na view.
    const DATE_COL = "sale_date";

    const MAIN_QUERY = `
      WITH
      sem AS (SELECT UNNEST($1::int[])::int AS semana),

      dias_calendario AS (
        SELECT c.data::date AS data, c.semana
        FROM calendario c
      ...
      base_vendedores AS (
        SELECT DISTINCT
          dl.loja,
          COALESCE(vv.seller_name, vd.seller_name) AS seller_name
        FROM vw_fiscal_itens_com_movimento vv
        JOIN periodo p
          ON vv.${DATE_COL}::date BETWEEN p.dt_ini AND p.dt_fim
        JOIN d_loja dl
          ON dl.branch_code = vv.branch_code
        LEFT JOIN vendedores vd
          ON vd.seller_code = vv.seller_code
        WHERE ($3::text[] IS NULL OR dl.loja = ANY($3::text[]))
          AND ($4::text[] IS NULL OR COALESCE(vv.seller_name, vd.seller_name) = ANY($4::text[]))
      ),
      ...
      vendas AS (
        SELECT
          ...
          SUM(vv.net_value)::numeric(18,2) AS realizado
        FROM vw_fiscal_itens_com_movimento vv
        JOIN periodo p
          ON vv.${DATE_COL}::date BETWEEN p.dt_ini AND p.dt_fim
        JOIN calendario c
          ON c.data::date = vv.${DATE_COL}::date
        JOIN d_loja dl
          ON dl.branch_code = vv.branch_code
        LEFT JOIN vendedores vd
          ON vd.seller_code = vv.seller_code
      ...
      SELECT
        ...
        CASE WHEN SUM(meta_vendedor) > 0
             THEN (SUM(realizado)/SUM(meta_vendedor))*100
    ...
    // log para garantir que não existe invoicedate na query
    console.log("[api/comissao] MAIN_QUERY contains invoicedate?", MAIN_QUERY.includes("invoicedate"));
    ...
    return res.status(200).json({
      ...
      subtotais_loja,
      total_geral,
      datecol: VV_DATE_COL, // ainda devolvo, mas SQL usa sale_date de todo jeito
    });
  } catch (err) {
    console.error("Erro API comissao:", err);
