...
// Usar SEMPRE a coluna issuedate da view
const VV_DATE_COL = "issuedate";
...
export default async function handler(req, res) {
  try {
    ...
    const MAIN_QUERY = `
      ...
      dias_semana AS (
        SELECT c.ano, c.semana, c.data::date AS data
        FROM calendario c
      ...
      base_vendedores AS (
        ...
        JOIN dias_semana d
          ON d.data   = vv.${VV_DATE_COL}::date
         AND d.semana = EXTRACT(WEEK FROM vv.${VV_DATE_COL})::int
         AND ($2::int[] IS NULL OR d.ano = ANY($2::int[]))
        JOIN d_loja dl              ON dl.branch_code    = vv.branchcode
        LEFT JOIN vendedores vd     ON vd.seller_code    = vv.dealercode::int
        WHERE EXTRACT(WEEK FROM vv.${VV_DATE_COL})::int = ANY($1::int[])
          AND ($3::text[] IS NULL OR dl.loja        = ANY($3::text[]))
          AND ($4::text[] IS NULL OR vd.seller_name = ANY($4::text[]))
      ),
      grade AS (
        SELECT
          b.loja, b.seller_name, m.semana,
          m.meta_semana_loja, m.qtd_vendedor,
      ...
      vendas AS (
        SELECT
          dl.loja AS loja,
          vd.seller_name,
          EXTRACT(WEEK FROM vv.${VV_DATE_COL})::int AS semana,
    ...
    const SUBTOTAL_LOJA_QUERY = `
      ...
      linhas AS (
        SELECT
          g.loja,g.seller_name,g.semana,g.meta_vendedor,COALESCE(v.realizado,0) AS realizado,
          (${COMISSAO_SQL})::numeric(6,4) AS taxa,
          (COALESCE(v.realizado,0)*(${COMISSAO_SQL}))::numeric(18,2) AS comissao
        FROM grade g
        LEFT JOIN vendas_vend v
          ON v.loja=g.loja AND v.seller_name=g.seller_name AND v.semana=g.semana
      ),
      com_agg AS (
    ...
    // Execução
    const paramsMain   = [semanasISO, anos.length ? anos : null, lojas.length ? lojas : null, vendedores.length ? vendedores : null];
